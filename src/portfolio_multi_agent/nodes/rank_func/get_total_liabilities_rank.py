...
async def get_total_liabilities_rank(
    top_n: int = 30,
    app_key: str = "",
    app_secret: str = "",
    access_token: str = "",
    ...
    Returns:
        부채총계 순위 상위 N개 종목 정보 리스트 [{"code": "종목코드", "name": "종목명"}, ...]
    """
    _app_key = app_key
    ...
    url = f"{base_url}/uapi/domestic-stock/v1/ranking/profit-asset-index"
    headers = {
    ...
    # Output: 수익자산지표 순위 리스트
    profit_asset_rank_list = api_response.get("output", [])

    # 상위 N개의 종목 정보 추출 (중복 제거, 순서 유지)
    seen = set()
    stock_list = []
    for stock in profit_asset_rank_list[:top_n]:
